...
function buildInterpretation(weather, t) {
  if (!weather) return t('weatherDetail.interpretation.loading');
  if (weather.rainChance >= 70) return t('weatherDetail.interpretation.rainHigh');
  if (weather.humidity <= 40) return t('weatherDetail.interpretation.dry');
  if (weather.condition.toLowerCase().includes('nubl')) return t('weatherDetail.interpretation.cloudy');
...
export default function WeatherDetailPage({
  ...
  return (
    <main
      data-testid="weather-detail-page"
      className="weather-detail-page min-h-screen bg-[radial-gradient(circle_at_top,_rgba(220,231,199,0.55),_rgba(247,242,231,0.95)_32%,_#efe2cf_100%)] px-4 pb-14 pt-6 text-earth-900"
    >
      <div className="mx-auto max-w-md space-y-5">
        ...
        <section className="space-y-4">
          ...
          <Card className="bg-[linear-gradient(160deg,_rgba(255,255,255,0.96)_0%,_rgba(244,239,229,0.96)_100%)]">
        ...
        <section className="space-y-4">
          ...
          <Card className="bg-[linear-gradient(160deg,_rgba(241,246,252,0.98)_0%,_rgba(255,255,255,0.94)_100%)]">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-earth-500">{t('weatherDetail.phase')}</p>
            <p className="mt-2 text-xl font-semibold text-earth-900">
              {lunarData.displayName || (recommendationData.lunarPhase
                ? t(`lunarPhases.${recommendationData.lunarPhase}`, { defaultValue: recommendationData.lunarPhase })
                : '—')}
            </p>
